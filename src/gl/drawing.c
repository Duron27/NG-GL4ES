...
void gl4es_glMultiDrawElements(GLenum mode, GLsizei* counts, GLenum type, const void* const* indices, GLsizei primcount)
{
    ...
    for (int i = 0; i < primcount; i++) {
        ...
        if (compiling) {
            // Handle compiling/renderlist when compiling
            renderlist_t* list = NULL;
            GLsizei min, max;
            ...
            // Normalize indices
            if (type == GL_UNSIGNED_INT) {
                // For GLuint, we might need to normalize to GLushort or process differently
                normalize_indices_ui(iindices, &max, &min, count);  // Modify for GLuint normalization
            }
            else {
                normalize_indices_us(sindices, &max, &min, count);
            }
            ...
            list = arrays_to_renderlist(list, mode, min, max + 1);
            list->indices = (type == GL_UNSIGNED_INT) ? (void*)iindices : (void*)sindices;
            list->ilen = count;
            list->indice_cap = count;
...
void gl4es_glMultiDrawElementsBaseVertex(GLenum mode, const GLsizei *counts, GLenum type, const void **indices, GLsizei primcount, const GLint * basevertex) {
    ...
    {
        gl4es_glDrawElementsBaseVertex(mode, counts[i], type, indices[i], basevertex[i]);
    }
}
void glMultiDrawElementsBaseVertex( GLenum mode, GLsizei *counts, GLenum type, const void * const *indices, GLsizei primcount, const GLint * basevertex) AliasExport("gl4es_glMultiDrawElementsBaseVertex");

    ...
    switch(internalformat) {
        ...
        case GL_DEPTH_COMPONENT:
            *format = GL_DEPTH_COMPONENT;
            if (*type != GL_UNSIGNED_SHORT && *type != GL_FLOAT) {
        ...
        case GL_R32F:
            ...
            *type = (hardext.floattex)?GL_FLOAT:GL_UNSIGNED_BYTE;
            break;
        default:
            SHUT_LOGD("LIBGL: Warning, unknown Internalformat (%s)\n", PrintEnum(internalformat));
            *format = GL_RGBA;
            *type = GL_UNSIGNED_BYTE;
            break;
    }
...
//    SHUT_LOGD("format(internal)=%s(%s), type=%s data=%s \n", PrintEnum(format), PrintEnum(internalformat), PrintEnum(type), (data==NULL)?"false":"true"); 
    ...
    if (GL4ES_AUTOMIPMAP_PLACEHOLDER) {
        if (level>0)
            if ((GL4ES_AUTOMIPMAP_PLACEHOLDER==1) || (GL4ES_AUTOMIPMAP_PLACEHOLDER==3) || bound->mipmap_need) {
        ...
        if(bound->npot) {
            if(target==GL_TEXTURE_RECTANGLE_ARB && hardext.npot)
                limitednpot=1;
                 ...
                 || (GL4ES_AUTOMIPMAP_PLACEHOLDER==3) 
                 || (GL4ES_AUTOMIPMAP_PLACEHOLDER==4 && width!=height) 
...
#define NO_1x1
        ...
        if ((GL4ES_AUTOMIPMAP_PLACEHOLDER==4) && (nwidth!=nheight))
            bound->mipmap_auto = 0;
        ...
        if (!(globals4es.texstream && bound->streamed)) {
            if ((target!=GL_TEXTURE_RECTANGLE_ARB) && (GL4ES_AUTOMIPMAP_PLACEHOLDER!=3) && (bound->mipmap_need || bound->mipmap_auto) && !(bound->npot && hardext.npot<2) && (bound->max_level==-1)) {
            ...
            if(GL4ES_AUTOMIPMAP_PLACEHOLDER==5 && !level)
                bound->mipmap_done = 0;
            // check if max_level is set... and calculate higher level mipmap
            if(((bound->max_level == level && (level || bound->mipmap_need)) || (callgeneratemipmap && level==0) || (GL4ES_AUTOMIPMAP_PLACEHOLDER==5 && level && !bound->mipmap_done)) && !(bound->max_level==bound->base_level && bound->max_level==0)) {
                if(GL4ES_AUTOMIPMAP_PLACEHOLDER==5 && level==1)
                    bound->mipmap_done = 1;
...
/*	// copy the texture to the buffer
    ...
    /*if (bound->mipmap_need && !bound->mipmap_auto && (GL4ES_AUTOMIPMAP_PLACEHOLDER!=3) && (!globals4es.texstream || (globals4es.texstream && !bound->streamed)))
        gles_glTexParameteri( rtarget, GL_GENERATE_MIPMAP, GL_FALSE );*/
...
{
/*
if(isFormatSupported(internalformat) && globals4es.dxt != 1)
{
